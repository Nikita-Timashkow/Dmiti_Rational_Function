class Natural:
    ...
    def DIV_NN_Dk(self, other) -> (int, int):
        ...
        # Определяем позицию k для первой цифры
        k = self.len - other.len
        if k < 0:
            return 0, 0

        #  Создаем Natural для k
        if k < 10:
            k_natural = Natural(0, [k])
        else:
            k_digits = [int(d) for d in str(k)]
        ...
        # Умножаем делитель на 10^k (сдвигаем влево на k разрядов)
        ...
        for digit in range(9, 0, -1):
            ...
            if self.COM_NN_D(temp) != -1: # если self >= temp
                return digit, k # нашли максимальную цифру
